...
public class UserAuthorizationController {
    ...
    public ResponseEntity<PersistentEntityResource> authorize(@RequestParam String code, Authentication auth, PersistentEntityResourceAssembler asm) {
    ...
    public ResponseEntity<PersistentEntityResource> authorize(@RequestParam Integer vkId, @RequestParam String vkToken, Authentication auth, PersistentEntityResourceAssembler asm) {
        return vkService.authorize(vkId, vkToken)
                .map(actor -> processAuthorization((Integer) auth.getPrincipal(), actor.getId(), actor.getAccessToken(), asm))
                .orElseThrow(() -> new IllegalArgumentException("Wrong userId or token!"));
    }

    private ResponseEntity<PersistentEntityResource> processAuthorization(Integer tgId, Integer vkId, String vkToken, PersistentEntityResourceAssembler asm) {
        try {
